function addCountyAndCensusToAddrModel(vCapId) {
	...
	var query2Resource = "/arcgis/rest/services/Government/CACounties10/MapServer/0/query";
	var query2Params = "geometry=%%XCOORD%%,%%YCOORD%%&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=false&f=json";
	...
	if (vAddrModelArry.getSuccess()) {
		...
		for (x in vAddrModelArry) {
			...
			if (postResult.getSuccess()) {
				...
				if (jsonCandidates.length > 0) {
					...
					// Use X and Y to return Unincorporated Info
					query2Params = query2Params.replace("%%XCOORD%%", "" + Xcoord);
					query2Params = query2Params.replace("%%YCOORD%%", "" + Ycoord);
					QPostResult = aa.util.httpPost(geoCodeURL + query2Resource, query2Params);
					if (QPostResult.getSuccess()) {
						//logDebug("Post 2 Successful");
						QJsonOutput = QPostResult.getOutput();
						QJsonObject = JSON.parse(QJsonOutput);
						...
						var unincorp = (features.length == 0);
						...
						if (addressAttrObjArry != null) {
							...
							for (y in addressAttrObjArry) {
								addressAttrObj = addressAttrObjArry[y];
								if (addressAttrObj.getName() == "UNINCORPORATED") {
									addressAttrObj.setB1AttributeValue(unincorp ? "YES" : "NO");
									logDebug("Setting address attribute " + "UNINCORPORATED" + " to " + unincorp);
								} 
							}
						} else {
							logDebug("No address attributes found");
		...
		logDebug("Error getting CAP addresses. " + vAddrModelArry.getErrorMessage());
	}
}
