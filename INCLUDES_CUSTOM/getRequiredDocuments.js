function getRequiredDocuments(isPageFlow) {
	...
	var documentationOfLocalCompliance = {
		...
		workflow: wfStopAll
	}; // 5006(b)(23)  updated language for us 3056
	var laborPeaceAgreement = {
		condition: "Labor Peace Agreement or Notarized Statement",
		document: "Labor Peace Agreement or Notarized Statement",
		workflow: wfStopPermanentOnly
	}; // 5006(b)(26)
...
//	};
	...
	// Added in US 2905
	var cannabisWasteProcedures = {
		condition: "Cannabis Waste Procedures",
		document: "Cannabis Waste Procedures",
		workflow: wfStopPermanentOnly
	};
	// Added in US 2905
	var deliveryProcedures = {
		condition: "Delivery Procedures",
		document: "Delivery Procedures",
		workflow: wfStopPermanentOnly
	};
	// Added in US 3060
	var productQualityPlan = {
		condition: "Product Quality Plan",
		document: "Product Quality Plan",
		workflow: wfStopPermanentOnly
	};
	...
	// Added in US 3063
	var foreignBusiness = {
		condition: "Foreign Business Document",
		document: "Foreign Business Document",
		workflow: wfStopPermanentOnly
	}; 
	/*------------------------------------------------------------------------------------------------------/
	...
	var hasRetailerActivity = isASITrue(AInfo["Retailer"]); // See user story 2905
	var hasRetailerNonstorefrontActivity = isASITrue(AInfo["Retailer Non-Storefront"]); // See user story 2905
	...
	var isForeignBiz = false; //See US 3063
	...
	if (isAttestationAmendment) {
		//get the business contact from the parent for US 3063
		var business = getContactObj(parentCapId, "Business");
	} else {
		//get the business contact from the record
	...
	if (business && business.asi) {
		...
		// Added in US 3063
		isForeignBiz = ((business.asi["What is your business's organizational structure?"] == "Foreign Corporation") || (business.asi["What is your business's organizational structure?"] == "Foreign Limited Liability Company"));
	}
	...
	if ((isApplication || isAttestationAmendment) && !isOwnerAttestation && !isEventOrganizer && !isCEOAttestation && !isTemporaryEvent && !isTemporaryEventAttestation) {
		...
		if (!isTemporaryRequest) {
			...
			//if (isDistributor || isRetailer || isRetailerNonStore || isMicroBusiness || isDistribTransportOnly || isTesting) {
				// Added in US 2905  Removed in US 3057
			//	requirementArray.push(cannabisWasteProcedures);
			//}
			...
			if (isRetailer || isRetailerNonStore || (isMicroBusiness && (hasRetailerActivity || hasRetailerNonstorefrontActivity))) {
				// Added in US 2905
				requirementArray.push(deliveryProcedures);
			}
	...
	if (isBusiActCult && matches(isCultLicType,"Small Indoor", "Small Mixed-Light Tier 1", "Small Mixed-Light Tier 2", "Specialty Cottage Indoor", "Specialty Cottage Mixed-Light Tier 1", "Specialty Cottage Mixed-Light Tier 2", "Specialty Indoor", "Specialty Mixed-Light Tier 1", "Specialty Mixed-Light Tier 2"  )) {
			...
			if (isBusiActMfct) {
				...
				requirementArray.push(productQualityPlan);  //added with us 3060
			}
...
//taken out of story 2375
		...
		//	}
				...
				if (isBusiActMfct && (isCanExtract || isInfusionOper)) {
					// Start ETW - Removed per defect 2927
					// requirementArray.push(standardOperatingProcedures);
					// End ETW - Removed per defect 2927
					requirementArray.push(listOfCannabisProductsManufactured);
					requirementArray.push(wasteManagementProcedures);
				}
			
			// Add foreign business doc per US 3063
			if (isForeignBiz) {
				requirementArray.push(foreignBusiness);
			}
