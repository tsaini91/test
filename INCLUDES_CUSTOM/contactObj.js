...
logDebug("ETW: new 9.0 contactObj function.");
    ...
    if (ccsm)  {
		...
		this.updateSyncFlag= function(boolFlag) {
			if (this.capContact) {
				this.capContact.setSyncFlag(boolFlag ? "Y" : "N");
				var result = aa.people.updateCapContactSyncFlag(this.capContact);
				logDebug("Setting sync flag to " + boolFlag + ", success? " + result.getSuccess());
				}
